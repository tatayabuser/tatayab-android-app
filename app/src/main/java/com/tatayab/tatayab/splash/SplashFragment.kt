...
import com.tatayab.tatayab.listener.OnCityListener2
...
class SplashFragment : BaseFragment(), NavigationResult, OnCountryListener , OnCityListener , OnCityListener2 {
...
//                    main_progress.visibility = View.VISIBLE
    ...
    fun initCities(){
        if(mmmcitiesList.isNullOrEmpty()) {
            mmmcitiesList.add(CityModel(1,1,"Riyadh","الرياض","1",false))
            mmmcitiesList.add(CityModel(2,2,"Jeddah","جده","2",false))
            mmmcitiesList.add(CityModel(3,3,"Madinah","المدينه","3",false))
            mmmcitiesList.add(CityModel(4,4,"Dammam","الدمام","4",false))
            mmmcitiesList.add(CityModel(5,5,"Khobar","الخبر","5",false))
            mmmcitiesList.add(CityModel(6,6,"Mecca","مكه","6",false))
            mmmcitiesList.add(CityModel(7,7,"Hasa","الاحساء","7",false))
        ...
        btn_start.setSafeOnClickListener {
            try {
                when {
                    ...
                    SelectedCountry.country == saCountry && mCity == null-> {
                        view?.showSnackbar(
...
//        countriesAdapter.setItems(countriesItems)
    ...
    override fun onCitySelectedd(city: CityModel) {
        if(city.code == "0"){
            goToCitiesListScreen(App.getPref().currentLanguage.toString(),saCountry?.code.toString())
            mCity=city.name_ar
            /* citiesList?.let {
