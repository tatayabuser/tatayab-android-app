...
//        return if (ENABLE_GRAPH_QUERIES_CALLS) {
        var product_id = 0
        if (addToCartRequest?.product_id.isNullOrBlank().not()) product_id =
            addToCartRequest?.product_id?.toInt()!!
...
//            service.getProductsForCategory(options, page, itemPerPage)
    ...
    override fun getProductGift(
    ): Flowable<ProductsListResponse> {
//        return if (ENABLE_GRAPH_QUERIES_CALLS) {
        return graphService.getGiftItems(GraphRequest(getGiftItems()))
            .flatMap {
